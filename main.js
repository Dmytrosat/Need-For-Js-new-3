...
const keys = {
  //название клавиш для управления
  ArrowUp: false,
  ArrowDown: false,
  ArrowRight: false,
  ArrowLeft: false,
...
const setting = {
  //первоначальные данные, запущена игра или нет
  start: false,
  score: 0,
  speed: 3,
...
function getQuantityElements(heightElement) {
  //принимает параметр количество элементов
...
function startGame() {
  ...
  for (let i = 0; i < getQuantityElements(100); i++) {
    //линии в середине дороги, c высотой 100px
  ...
  for (let i = 0; i < getQuantityElements(100 * setting.traffic); i++) {
    //добавление друг машин, растояние машин зависит от трафика
    const enemy = document.createElement("div");
    enemy.classList.add("enemy");
    enemy.y = -100 * setting.traffic * (i + 1); // 100высота машины, i расстояние между машинами
    enemy.style.left =
      Math.floor(Math.random() * (gameArea.offsetWidth - 50)) + "px"; //располож лругих машина по горизонтали, 50чтобы от краев не уходило
    enemy.style.top = enemy.y + "px"; //расстояние от верха игрового простр
    enemy.style.background =
      'transparent url("./image/enemy3.png") center / cover no-repeat'; //цвета других машин
...
function playGame() {
  ...
  if (setting.start) {
    ...
    if (keys.ArrowRight && setting.x < gameArea.offsetWidth - car.offsetWidth) {
      //(ширина дороги - ширина машины), упирается в правый край
      setting.x += setting.speed;
    }

    if (keys.ArrowUp && setting.y > 0) {
      //по оси -y вверх, y > 0 когда упирается верх края
...
function moveRoad() {
  ...
  lines.forEach(function (line) {
    //forEach метод перебора, line-элемент
    line.y += setting.speed; //3px
    line.style.top = line.y + "px";

    if (line.y >= document.documentElement.clientHeight) {
      //clientHeight высота стр
...
function moveEnemy() {
  ...
  enemy.forEach(function (item) {
    ...
    if (item.y >= document.documentElement.clientHeight) {
      //чтобы машины не закаончивались на экране
      item.y = -100 * setting.traffic;
      item.style.left =
        Math.floor(Math.random() * (gameArea.offsetWidth - 50)) + "px"; //др машины выходили рандомна сверху
    }
  });
}
